...
const BaseLayout = ({children}: { children?: JSX.Element | [JSX.Element] }) => {
    ...
    switch (appState) {
        ...
        case AppStates.Errored:
            selectedLayout = <ErrorTab message={errorMessage} description={Strings.ErrorRefresh} onClick={() => window.location.reload()}/>
            break
        default:
            selectedLayout = <div className={styles.container}>
                <Header/>
                {children}
            </div>
    }
